...
def call_augfn_torch_batched(sample_batch, aug_fn, copies=1):
    """
    Sample: pytorch array of unit8 of (B, 9, H, W)
    aug_fn: expects torch tensor normalized using normalize()
    """
    batch_size, nine, H, W = sample_batch.shape
    assert nine == 9

    if copies == 1:
        with torch.no_grad():
            X = sample_batch.reshape((1, batch_size * 9, H, W))
            X = aug_fn(X).clamp(0, 1)
            X = X.reshape((batch_size, 9, H, W))
        return X
    else:
        with torch.no_grad():
            Xs = []
